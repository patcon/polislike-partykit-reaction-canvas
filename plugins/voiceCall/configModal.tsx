import { useEffect, useState } from 'react';
import { useAdminSocket } from '../../app/components/panels/AdminPanelNoDB/AdminSocketContext';

const ALGORITHM_OPTIONS: { value: string; label: string; description: string }[] = [
  { value: 'first-available', label: 'First Available', description: 'Paired instantly with the longest-waiting participant (FIFO)' },
];

export default function VoiceCallConfigModal({ onClose }: { onClose: () => void }) {
  const { send, subscribe, getLastMessage } = useAdminSocket();
  const [callAlgorithm, setCallAlgorithm] = useState<string>(
    () => (getLastMessage('voiceCallState')?.callAlgorithm as string) ?? 'first-available',
  );

  useEffect(() => {
    return subscribe(data => {
      if (data.type === 'voiceCallState' && data.callAlgorithm) {
        setCallAlgorithm(data.callAlgorithm as string);
      }
    });
  }, [subscribe]);

  const handleSelect = (algorithm: string) => {
    setCallAlgorithm(algorithm);
    send({ type: 'setCallAlgorithm', algorithm });
  };

  return (
    <div className="app-modal-overlay" onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="app-modal">
        <p className="app-modal-title">Voice calls settings</p>

        <p style={{ color: '#ccc', fontSize: 14, fontWeight: 600, marginBottom: 8 }}>Matching Mode</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 20 }}>
          {ALGORITHM_OPTIONS.map(({ value, label, description }) => (
            <label key={value} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, cursor: 'pointer' }}>
              <input
                type="radio"
                name="callAlgorithm"
                value={value}
                checked={callAlgorithm === value}
                onChange={() => handleSelect(value)}
                style={{ marginTop: 2, cursor: 'pointer', flexShrink: 0 }}
              />
              <span>
                <span style={{ color: '#ccc', fontSize: 14 }}>{label}</span>
                <span style={{ display: 'block', color: '#666', fontSize: 12 }}>{description}</span>
              </span>
            </label>
          ))}
        </div>

        <button className="app-modal-btn-dismiss" onClick={onClose}>Close</button>
      </div>
    </div>
  );
}
