...
### Added
- **MapViewerPanel: color-by-moment** — gear icon in Interfaces tab opens a config modal; choose "None" (uniform dots) or "Moment" (select a moment to color participants by their vote: green=agree, red=disagree, yellow=pass, gray=missing). Active moment name and color legend shown in the panel header.
