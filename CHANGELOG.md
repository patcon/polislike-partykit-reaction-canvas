...
### Added
- **Story Tracer: reducer algorithm selector** — choose between four dimensionality reduction algorithms via a new dropdown in the Story Tracer settings: UMAP (umap-js, default), UMAP (DruidJS), LocalMAP, and PaCMAP. The selected algorithm is persisted to localStorage and stored alongside the computed points' metadata. All algorithms run inside the existing Web Worker. ([#94](https://github.com/patcon/polislike-partykit-reaction-canvas/issues/94))
