...
- V4 admin: Participants tab — lists all connected users grouped by their current valence zone (using the active label set for group headings); users with no active cursor appear under "Lurking"; flat list available via grouping dropdown; row layout includes a disabled action button placeholder for future per-user actions ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
### Fixed
- V4 admin: fix vertical bounce when swiping the tab bar on iOS/Android — added `touch-action: pan-x` and `overscroll-behavior-x: contain` so the browser treats tab-bar swipes as horizontal-only ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
### Added
- Onboarding: added `valence-onboarding-v1.html` and `valence-onboarding-v2.html` to `public/` and linked both from the IndexApp landing page ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: increase cursor radius slider max from 8 to 30; fix line width slider by switching chord lines from `LineBasicMaterial` (WebGL ignores linewidth) to `LineSegments2`/`LineMaterial` (geometry-based thick lines) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: add "2d time series" view mode with fading cursor trace lines; "2d" renamed to "2d time slice"; trace style column in style grid now active in time-series and 3d views ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: change default cursor radius to 10 and line width to 2.0 ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: add "linear" geometry mode — all traces collapse to a single vertical line at centre; animated transition to/from parallel ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: add "parallel" geometry mode — chords become vertical lines equally spaced across 2×R, positive (green) ends gather at top, negative (red) at bottom; animated transition to/from diametric; sequential/simultaneous animation modes work as expected ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
- Onboarding V2: WebSocket connection panel — room input, connect/disconnect button, and live status; connected participants each appear as an additional blue (#4285F4) chord on top of simulated ones, with trace and fill surfaces; cursor color follows group/valence style setting; chord entry/exit animations run even when paused; cursor dot continues updating from live WS data while paused; group palette reordered so blue appears last (index 6) to keep live-user blue visually distinct ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: fill surface — a translucent wedge mesh connecting each chord's trace path back to the canvas origin along the time (Z) axis, coloured and faded like the trace; wired up the fill column in the style grid (group/valence buttons now active) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: per-element opacity sliders in the style grid (radial / trace / cursor / fill), replacing the single global opacity slider; trace and fill sliders dim alongside their other controls when in 2d time slice mode ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))

### Fixed
- Onboarding V2: fix z-fighting glitch between guide lines and radial dots by setting `depthWrite:false`/`depthTest:false` on guides and using `renderOrder` (guides=-1, dots=10) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: fix 3D scroll-to-zoom (wheel on desktop, pinch on mobile); zoom now persists correctly instead of being overwritten each frame ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: fix "origin v2" exit — chords now animate from their actual rendered position; fixes `departN` off-by-`idx` formula and group-slot position mismatch on departure ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: fix "origin v2" entry/exit symmetry — new chords start at their group target value so entry is a pure scale-up (no post-animation drift); exit was already pure scale-down ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: in "none" enter/exit mode, new chords now appear immediately at their group target value instead of animating in from near-zero ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: "origin v2" entry/exit animations are now symmetric (both use ease-in: `t²` / `1-t²`) and 3.75× slower (30 frames instead of 8) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: fix group-slot arc distribution in persistent mode — slices are now Fibonacci-proportional (matching `assignBase` weights) instead of equal `1/ng`, so chord density is uniform across groups in all geometry modes ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: fix fill surface origin — fill now anchors to each chord's historically-correct root position (stored alongside tip in history) instead of always the global canvas centre; fixes parallel mode where each chord has a distinct root x-position, and correctly tracks the root through geometry transitions ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Onboarding V2: fix black chord/dot outlines appearing on fill surface when radial/cursor opacity is 0 — chord and dot materials now have `depthWrite:false`, consistent with trace and fill materials ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))

## Week 19 (2026-03-30)

### Added
- Experience: Valence Viz — facilitator tool (`valence-viz.html`) wrapping the Three.js particle/wave valence visualization. Runs synthetic data by default (scrub bar + play/pause). "Audience Sync" toggle connects to a PartyKit room via WebSocket and drives valence values from live cursor positions using barycentric region weighting. Supports light-wave and charged-particle modes, group/valence coloring, and orbit camera. Index card added to landing page. ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Experience: Mood Sounds — facilitator tool (`mood-sounds.html`) for ambient generative sound tied to live audience cursor positions. Connects as admin (invisible to participant count). Audience Sync toggle drives the mood slider from WebSocket cursor data; Valence toggle switches between Continuous (raw x-position average) and Unit (each cursor snapped to zone −1/0/+1 before averaging) modes. Room input updates the `?room=` URL param for shareability. ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Index: added card linking to Mood Sounds. ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
- Valence Viz: **camera mode** cycle button (click to rotate through `static` → `lerp` → `exp` → `spring` → `quat`); `static` preserves the original snap-to-position behaviour; the four smooth modes gradually follow the path — simple lerp, frame-rate-independent exponential decay, critically-damped spring physics, and exponential-decay position with quaternion slerp for rotation ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Valence Viz: spacebar toggles play/pause ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Valence Viz: **target mode** toggle button (`head` / `trail`) — `head` keeps the existing orbit-around-cursor-head behaviour; `trail` positions the camera 80 steps behind the cursor head looking forward down the path; independent of the smoothing mode button so any combination works ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Valence Viz: increase simulation steps from 1000 to 1600 (`LIVE_STEPS` 800→1280, `HISTORY_STEPS` 200→320) so traces are denser along the longer semantic path; also switch semantic CatmullRom to arc-length parametrization (`getPointAt`/`getTangentAt`) so steps are spatially uniform rather than clustering near dense control points ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Valence Viz: set `radiusScale=1.0` for semantic path mode (was 0.1) so tube diameter, cursor scale, and effective camera distance are consistent with simple curves mode ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Valence Viz: increase max zoom-out distance from 5.0 to 15.0 world units ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Valence Viz: increase semantic path spread from 3.6 to 20.0 world units so tube diameter is proportionally smaller relative to the path ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Valence Viz: fix trails/fills/tube disappearing at large path scale by setting `frustumCulled = false` on all dynamic geometry — Three.js was culling objects whose stale bounding sphere no longer intersected the frustum after switching to the larger semantic path ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
- Valence Viz: timeline scrubber can now be dragged while playing (not just while paused); playback resumes from the scrubbed position on release ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- All versions: `crypto.randomUUID()` now falls back to a `Math.random`-based UUID v4 on non-secure contexts (e.g. accessing the dev server via a local network IP over plain HTTP), fixing a crash on load ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
- CI: PR preview deploy and cleanup workflows now use `npx partykit` instead of bare `partykit`, which isn't on PATH after `npm ci` ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- CI: PR preview workflow now has `pull-requests: write` permission so it can post the preview URL comment ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
### Added
- V1/V2/V3/V4: `?room=` is now the canonical param for setting the PartyKit room across all apps; `?videoId=` is a deprecated alias that still works for backward compatibility; in V2 `?room=` also sets the YouTube video ID ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2/V3/V4: presence counter now shows "X here · Y touching" (plus ▶️/⏸️ play state on V2) — participant count and active touching count (others' cursors + own touch if active) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2: paused overlay now shows a QR code beside the instructional text so others can scan to join; QR code encodes the current page URL ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2: when the video is paused (no one touching the canvas), a semi-transparent overlay on the video area instructs users to put their finger on the space below to start and keep the video playing ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V4 admin: recorded events are displayed live in a table below the recording controls; table shows the last 200 events with columns for row number, timestamp, connectionId, from/to (transitions mode) or type/x/y (positions mode) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V4 admin: "Download JSON" button is now separate from stopping — it appears after recording has stopped and events are present; events accumulate across multiple recording sessions until cleared ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V4 admin: "Clear" button erases all accumulated events and resets the recording session start time ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- All admin panels: connections from admin pages are excluded from the participant presence count shown to canvas users; admin panels pass `isAdmin=true` on their WebSocket connection; the server tracks these separately and filters them out of `presenceCount` broadcasts ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
- V2/V4/V5: `?labels=` URL param now overrides the admin-set room label for that participant only (previously it was only a fallback when no server label was set) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V4/V5 Admin: destructive buttons now use consistent bright-red style (`v3-admin-btn--destructive`) instead of dark maroon inline styles ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V4/V5 Admin: previously hidden buttons (Download JSON, Clear, Clear all recordings) are now always visible and shown as disabled when unavailable ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Index page: cards now sorted reverse-chronologically (V5 → V4 → V2 → V1), with participation cards before admin cards within each version ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Index page: YouTube app cards (V2, V5) now have a red-tinted background to distinguish them visually ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Index page: removed blank YouTube cards (V2 and V5 without a video set) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Default example video updated to `irc6creOFGs` across all apps (landing page cards and no-video placeholder links) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2/V5: "example" link in the no-video placeholder now preserves existing query params (e.g. `?forceView=mobile`) when navigating to the example video ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
- V5 Admin: "Clear all recordings" button is hidden for rooms listed in `PROTECTED_ROOMS` (currently `irc6creOFGs`) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Supabase credentials now correctly embedded in `partykit.json` `define` block; `npx partykit env add` does not reach the client build and was ineffective ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V5: YouTube video now loads on arrival (missing `videoId` in `YT.Player` constructor) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V5: touching the canvas now plays the video; lifting pauses it (single-user touch-to-play) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V5: live cursors from other connected users are no longer shown; only Supabase replay cursors appear ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Canvas: new `hideCursors` prop suppresses cursor rendering while keeping labels/anchors sync active ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
- V3 participation and admin components deleted; `#v3` now redirects to `#v4` and V3 cards removed from the app landing page ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
- V2 mobile-only gate: non-touch devices see a QR code linking to the current URL instead of the canvas; override with `?forceView=mobile` ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
- V2: video plays only when every user in the room is touching the reaction canvas simultaneously; pauses as soon as anyone lifts their finger ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2: video timecode is saved to the server when any user lifts their finger; all clients seek to that position so playback resumes in sync next time everyone touches ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- TouchLayer: heartbeat re-sends current position every 2s while holding still, preventing Canvas's 3s staleness timeout from falsely dropping the cursor ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
### Changed
- All canvases: reaction label strings stored in title-case; displayed uppercase via CSS (`text-transform: uppercase`) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Label presets: removed `yesno` and `supportive`; added `atomic` (Attracted/Repelled/Neutral) and `valence` (Positive/Negative/Neutral); all presets have hint text ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Internal terminology: `VoteState`/`VoteRegion` → `ReactionState`/`ReactionRegion`; values `agree/disagree/pass` → `positive/negative/neutral`; `ReactionLabelSet` properties renamed to match; CSS semantic classes renamed (e.g. `.vote-label-agree` → `.reaction-label-positive`); Polis API fields (`agree_count` etc.) unchanged ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V4 help modal: label picker shows title-case values only (no machine key) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2 & V3: `?mobile=true` replaced by `?forceView=mobile` ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V3: new full-page reaction canvas variant (no video, no statements); supports `?room=`, `?labels=`, presence counter, blue-dot cursor, and mobile-only QR gate with `?forceView=mobile` override ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2: `?labels=` query param selects a reaction label preset; falls back to localStorage / default if omitted or unrecognised ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2: cursor room is now derived from `?videoId=` — each video gets its own cursor space automatically; `?room=` param removed from V2 (it remains a V1 concept) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
...
### Fixed
- Mobile taps no longer leave the cursor stuck as a persistent touch; the root cause was the browser's synthesized `mousemove` (fired ~300ms after every tap) landing in `handleMouseMove` and starting the heartbeat with no corresponding `mouseleave` to clean up — now suppressed with a 500ms post-touch guard ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2: video no longer auto-resumes on page refresh when it was previously playing; the iframe's `onLoad` now re-sends `pauseVideo` on non-touch devices (mobile browsers block autoplay natively so the guard is not needed there and was causing the player to go black) ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- V2: `seekTo` (triggered by timecode sync on lift) no longer unintentionally starts playback; a `pauseVideo` is sent immediately after every seek when not all touching ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
- Fix connection counter in V2 always showing zero; root cause was server-side presenceCount logic never having been committed/deployed ([`21256d2`](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/21256d2))
