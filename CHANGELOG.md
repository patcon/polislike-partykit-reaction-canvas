...
### Changed
- **Plugin context providers in V4 memoize their values** — `SocialMediaConfigProvider`, `GreeterConfigProvider`, and `ImageCanvasConfigProvider` now receive stable `useMemo`-wrapped value objects instead of inline object literals, preventing spurious re-renders of all context consumers on every parent render. `onConnectedAsViewer` callback also converted from an inline arrow to a `useCallback`.
- **Lazy-load non-V4 app variants and demos** — `client.tsx` now uses `React.lazy()` for V2, V5, ValenceViz, PerfCanvasApp, demos, and both front page variants; V4 remains a static import as the primary path. Each lazy boundary is wrapped in `<Suspense fallback={null}>`. Eliminates Three.js from the V4 participant bundle.
- **Tree-shake d3 in CursorField** — replaced `import * as d3 from 'd3'` with named `import { select } from 'd3'` and `import type { Selection } from 'd3'`, allowing the bundler to drop all unused d3 sub-packages (~450KB of the full d3 bundle) from the V4 participant path.
- **"Personal" chip renamed to "Push Screen" in the chip bar** — `KNOWN_CHIPS` now hardcodes `personal: 'Push Screen'` independently of `SOLO_SCREEN_LABEL`, which remains `'Personal'` for the emcee Interfaces tab column header.
- **InterfacesTab layout overhaul** — screen radio columns (Personal, Commons) moved to the far left under a "Screens" group header; Panel Share column renamed from "Share" and moved to far right; gear/settings button extracted from the Interface label cell into its own column at the end, dimmed when no config is available for that panel.
- **Commons screen added** — a second participant screen (`commons`) joins `personal`, both with independent panel selection from the emcee Interfaces tab. Screen panel state is generalized to a `Record<string, string>` map (`setScreenPanel`/`screenPanelChanged` messages gain a `screenName` field); plugin lifecycle hooks (`onActivate`/`onDeactivate`) remain scoped to the personal screen only. `?interface=commons` URL-privileges the commons chip (parallel to emcee); `?hideChipBar=true` hides the chip bar for fullscreen use. Header share icons in the Interfaces tab generate direct screen URLs (personal → `?interface=personal`; commons → `?interface=commons&hideChipBar=true`). Chip bar label: "Commons Screen".
...
- **Emcee Interfaces tab shows correct default radio when no panel has been explicitly set** — `useRoomConfig` now initializes `screenPanels` with `commons: 'canvas'` alongside `personal: 'canvas'`, and merges server state rather than replacing it on connect, so the canvas row is checked for both columns immediately on load.
- **Haptic no longer fires when a screen the participant can't see changes panel** — `screenPanelChanged` now only triggers the buzz if the changed screen is in the participant's unlocked interfaces (i.e. visible in their chip bar); changes to other screens are silently applied.
- **Gear icon in Interfaces tab is now consistent size and color when dimmed** — inactive gear was rendering smaller than the active state and at a different shade than the adjacent dimmed QR icon; both now use `color: '#333'` at default icon size.
...
- **Presence count and cursor state reset on canvas chip re-entry** — when `ReactionCanvasParticipant` is unmounted (switching to another chip) and remounted, the shared socket stays alive so the server never re-sends the `connected` message, leaving `CursorField` with stale defaults (`ownValenceDisplay: 'labels'`, zero presence count, etc.). `CursorField` now sends `getState` on mount; a new server handler re-sends the full `connected` snapshot plus a fresh `presenceCount` to the requesting connection.
- **Emcee's personal screen now updates when changing screen panels** — the `PANEL_REGISTRY` entry for 'Reaction Canvas' used `id: 'personal'`, colliding with the interface chip ID; clicking the Solo radio button called `sendScreenPanel('personal')` which overwrote the intended panel with a value nothing recognizes, causing the emcee's own screen to stay stuck. Fixed by renaming the registry entry to `id: 'canvas'` and updating the InterfacesTab settings-button guard accordingly.
- **Emcee can now receive pushed interfaces targeting themselves** — `getTargetConnections` in `party/server.ts` was unconditionally skipping admin connections, so pushing an interface to the emcee's own userId was silently dropped by the server. Removed the blanket admin exclusion; all connections are now reachable by userId, userIds, or region alike.
- **Push messages reach the emcee regardless of which interface chip is active** — `interfacePushed`, `pushedInterfacesCleared`, and `hapticPushed` were handled inside `CursorField`, which is only mounted on the personal interface chip; they are now handled in the always-mounted top-level `useMessageSubscription` in `ReactionCanvasAppV4Inner` and removed from the `CursorField` → `ReactionCanvasParticipant` prop chain.

### Changed
- **`Canvas.tsx` renamed to `CursorField.tsx`** — disambiguates the low-level cursor renderer from the many panel-level components that use "Canvas" in their names (`ReactionCanvas`, `ImageCanvas`, `ArrivalCanvas`, etc.). All imports, stories, and tests updated.
- **`activity` state replaced by `screenPanels` map; `ReactionCanvasParticipant` no longer always-mounted** — `ReactionCanvasAppV4` replaces the `activity: string` state with `screenPanels: Record<string, string>` (currently `{ canvas: panelId }`), making the single canvas screen explicit and extensible. Activity changes (`activityChanged` server message and initial `currentActivity` on connect) are now handled via a `useMessageSubscription` in the app shell rather than bubbling up through `Canvas` → `ReactionCanvasParticipant` callbacks, so the canvas component no longer needs to stay mounted when another interface is active. `ReactionCanvasParticipant` is now conditionally rendered only when the canvas interface is visible. `ActivityMode` type alias retired throughout app, party server, and plugins; all usages replaced with `string`.
...
- **`getPresenceCount` consolidated into `getState`** — `ArrivalCanvas` now sends `getState` on mount instead of the narrower `getPresenceCount`; the `getPresenceCount` server handler, client event type, and wire message are removed.
