...
- **Pluggable panel architecture** — introduces a `plugins/` directory with a `PanelPlugin<S>` interface that bundles panel metadata, an optional React component, an optional config modal, and optional server-side handlers (`createState`, `onConnect`, `onMessage`, `onActivate`, `onDeactivate`) into a single installable unit. Soccer is the pilot plugin: `SoccerPhysicsEngine` moves from `party/lib/soccerPhysics.ts` to `plugins/soccer/physics.ts`; server message routing is now plugin-driven via a pre-switch router; `InterfacesTab` now shows soccer settings via a gear-icon config modal (consistent with every other panel) instead of inline score/reset JSX; shared `BallState`/`SoccerScore` types and message-type constants live in `plugins/soccer/types.ts` for use by both client and server. ([feat/pluggable-panel-architecture](https://github.com/patcon/polislike-partykit-reaction-canvas/compare/main...feat/pluggable-panel-architecture))
- **`helloWorld` example plugin** — minimal reference implementation in `plugins/helloWorld/` covering all four plugin files (component, configModal, server, types); activated by uncommenting two lines in `plugins/index.ts`. Accompanied by `plugins/README.md` — a step-by-step authoring guide with field reference. ([c569167](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/c569167))
...
- **Greeter panel migrated to plugin system** — greeter is now a fully self-contained `PanelPlugin` in `plugins/greeter/`; server logic, component, and config modal are plugin-owned. Establishes the persisted-state pattern (`getPersistedState` / `applyPersistedState` hooks on `ServerPlugin`) for future panel migrations. InterfacesTab config gear now routes all plugin config modals through a single `setActiveConfigPlugin` handler instead of per-plugin open-state props.
- **Plugin config modals are now self-contained** — config modals call `useAdminSocket()` (a new React context from `AdminSocketContext.tsx`) to send messages and subscribe to socket events directly; `AdminPanelNoDB` renders any plugin's modal generically via `PLUGIN_MAP` lookup instead of hardcoded per-plugin conditionals; `PanelPlugin.configModal` type narrowed to `React.ComponentType<{ onClose: () => void }>`. ([70a5ea4](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/70a5ea4))
- **Plugins auto-register in the Interfaces tab** — `PANEL_REGISTRY` now spreads the `PLUGINS` array directly instead of listing each plugin by ID; adding a plugin to `plugins/index.ts` is sufficient for it to appear in the tab with no other changes. ([c569167](https://github.com/patcon/polislike-partykit-reaction-canvas/commit/c569167))
- **Rename `github-modal-*` CSS classes to `app-modal-*`** — removes the GitHub-specific prefix from the shared modal system; all config modals, push modals, and info dialogs now use the generic `app-modal-*` namespace.
- **`image-canvas` migrated to plugin system** — introduces `CanvasOverlay` as a new field on `PanelPlugin` for canvas-mode activities that configure the canvas rather than replacing it; `image-canvas` is the first consumer with a `background` component and `canvasProps` overrides; its config modal is now self-contained via `useAdminSocket()`, removing the `imageConfigOpen`/`roomImageUrl`/`sendImageUrl` prop chain from `useRoomConfig` → `AdminPanelNoDB` → `InterfacesTab`; all `activity === 'image-canvas'` hardcodes in `ReactionCanvasAppV4` replaced by plugin-driven reads from `PLUGIN_MAP`.
